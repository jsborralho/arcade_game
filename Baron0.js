...
var JSRoot = {
  SUPER: function (method) {
    return method.apply(this, Array.prototype.slice.apply(arguments).slice(1));
  },
  INIT: function () {
...
function NEW(clazz) {
  // Create an object and applies INIT(...) to it
  function F() {}
  F.prototype = clazz;
  var obj = new F();
...
function EXTENDS(clazz, added) {
  // Creates a subclass of a given class
  function F() {}
  F.prototype = clazz;
  var subclazz = new F();
  for (prop in added) subclazz[prop] = added[prop];
  return subclazz;
}
...
function globalByName(name) {
  // access global space entity by name (string)
  return this[name];
}
...
var GameImage = EXTENDS(JSRoot, {
  ...
  prefix: location.href + "/../resources/",
  next: function () {
    mesg("GameImage.next default");
  },
  ...
  INIT: function (kind, color, url, code) {
    ...
    if (url[0] == "@") url = this.prefix + url.slice(1);
    this.image.src = url;
    this.image.onload = function () {
      if (--GameImage.loading == 0) GameImage.next();
    };
